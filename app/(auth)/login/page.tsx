...
export default function Login() {
    ...
    const { handleGoogleLogin } = useGooleLogin();
            ...
            <div className="right-container max-w-full w-1/2 px-20 h-full flex items-center justify-center">
                <form className="w-[75%] max-w-3xl mx-auto flex flex-col gap-3" onSubmit={handleSubmit}>
                    <div className="flex flex-col gap-y-1">
                        ...
                        <div className="mt-4">
                            <button type="button" onClick={handleGoogleLogin} className="flex items-center justify-center gap-2 bg-[#EFF4FF] text-gray-800 text-xs font-normal px-4 py-2 rounded-md w-full h-10 hover:cursor-pointer"><FcGoogle size={15} /> Sign in with Google</button>
                        </div>
                    </div>
                    ...
                    <div>
                        <p className="text-center text-[12px] text-gray-500 mt-4">Don't have an account? <a href="/signup" className="text-indigo-500 hover:underline font-bold">Sign Up</a></p>
                    </div>
                </form >
            </div>
        </div>
